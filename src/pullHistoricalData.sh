...
--data-urlencode 'redirect_uri=https://127.0.0.1' >> ../docs/keys.json

REFRESH=$(grep -oP '"access_token" :\K[^,]*' ../docs/keys.json)
REFRESH=${REFRESH:2:-1}

input="../stockList1.txt"
...
--header 'Authorization: Bearer '${REFRESH} >> ../docs/exampleJSONData/${line}_historical_data.json
    echo ${line}
    python3 parseHistoricalJSON.py ../docs/exampleJSONData/${line}_historical_data.json
    aws s3 sync ../docs/exampleJSONData/${FILENAME} s3://fractal-data/
    rm ../docs/exampleJSONData/${line}_historical_data.json
done < $input
