#!/bin/bash

python3 ~/FractalOptions/src/parseHistoricalJSON.py DELETE

curl --location --request POST 'https://api.tdameritrade.com/v1/oauth2/token' \
--header 'Content-Type: application/x-www-form-urlencoded' \
--data-urlencode 'grant_type=refresh_token' \
--data-urlencode 'refresh_token=212FD3x19z9sWBHDJACbC00B75E' \
--data-urlencode 'client_id=H2PZBDOKOS3FLTGOPRBD0A9GX6UILCJL@AMER.OAUTHAP' \
--data-urlencode 'redirect_uri=https://127.0.0.1' >> ~/FractalOptions/docs/keys.json

REFRESH=$(grep -oP '"access_token" :\K[^,]*' ~/FractalOptions/docs/keys.json)
REFRESH=${REFRESH:2:-1}


end=$((SECONDS+1500))
input="/home/ubuntu/FractalOptions/stockList.txt"
while IFS= read -r line
do
    if [ $end -lt $SECONDS ];then
	rm ~/FractalOptions/docs/keys.json
	
	curl --location --request POST 'https://api.tdameritrade.com/v1/oauth2/token' \
	     --header 'Content-Type: application/x-www-form-urlencoded' \
	     --data-urlencode 'grant_type=refresh_token' \
	     --data-urlencode 'refresh_token=212FD3x19z9sWBHDJACbC00B75E' \
	     --data-urlencode 'client_id=H2PZBDOKOS3FLTGOPRBD0A9GX6UILCJL@AMER.OAUTHAP' \
	     --data-urlencode 'redirect_uri=https://127.0.0.1' >> ~/FractalOptions/docs/keys.json

	REFRESH=$(grep -oP '"access_token" :\K[^,]*' ~/FractalOptions/docs/keys.json)
	REFRESH=${REFRESH:2:-1}
	end=$((SECONDS+1500))
    fi
    
    curl --location --request GET 'https://api.tdameritrade.com/v1/marketdata/'${line}'/pricehistory?apikey=H2PZBDOKOS3FLTGOPRBD0A9GX6UILCJL&periodType=year&period=20&frequencyType=daily&frequency=1&needExtendedHoursData=false' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer '${REFRESH} >> ~/FractalOptions/docs/exampleJSONData/${line}_historical_data.json
    echo ${SECONDS}" "${end}
    echo ${REFRESH}
    python3 ~/FractalOptions/src/parseHistoricalJSON.py ~/FractalOptions/docs/exampleJSONData/${line}_historical_data.json
    aws s3 sync ~/FractalOptions/docs/exampleJSONData/${FILENAME} s3://fractal-data/
    rm ~/FractalOptions/docs/exampleJSONData/${line}_historical_data.json
done < $input

rm ~/FractalOptions/docs/keys.json

python3 ~/FractalOptions/src/generatePlot.py
